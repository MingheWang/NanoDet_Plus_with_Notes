...
def bbox_overlaps(bboxes1, bboxes2, mode="iou", is_aligned=False, eps=1e-6):
    ...
    if is_aligned:
        ...
        lt = torch.max(
            bboxes1[..., :, None, :2], bboxes2[..., None, :, :2]            # (m,1,2) , (1,n,2) ---> (m,n,2)
        )  # [B, rows, cols, 2]
        rb = torch.min(
            bboxes1[..., :, None, 2:], bboxes2[..., None, :, 2:]            # (m,1,2) , (1,n,2) ---> (m,n,2)
        )  # [B, rows, cols, 2]

        wh = (rb - lt).clamp(min=0)  # [B, rows, cols, 2]                   # (m,n,2) - (m,n,2) ---> (m,n,2)
        overlap = wh[..., 0] * wh[..., 1]
...
class GIoULoss(nn.Module):
    ...
    def forward(
        self,
        pred,                           # 正样本解码后的框的坐标  shape=(K,4)     4 = xyxy
        target,                         # 正样本对应的目标框坐标  shape=(K,4)     4 = xyxy
        weight=None,                    # 每个正样本的分类预测最大值  shape=(K,)
        avg_factor=None,                # m
        reduction_override=None,        
        **kwargs,
    ):
