...
class GhostModule(nn.Module):
    def __init__(
        ...
        self.oup = oup      # 96
        init_channels = math.ceil(oup / ratio)          # 96/2 = 48
        new_channels = init_channels * (ratio - 1)      # 48
        # point-wise逐点1X1卷积,通道数 192->48
    ...
    def forward(self, x):
        x1 = self.primary_conv(x)               # point-wise逐点1X1卷积,通道数 192->48
        x2 = self.cheap_operation(x1)           # 深度可分离卷积，特征图尺度不变，通道数：48->48
        out = torch.cat([x1, x2], dim=1)        # 通道方向链接：48+48 = 96
        return out
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(
        ...
        # 192->(1x1)->48                                            a
        # 深度可分离卷积，尺度不变：48->(3x3分离卷积)->48              a -> b
        # 48+48->96                                                 a + b
        self.ghost1 = GhostModule(in_chs, mid_chs, activation=activation)               

        # Depth-wise convolution
        if self.stride > 1:
            # 深度可分离卷积，尺度不变：96->(5x5分离卷积)->96
            self.conv_dw = nn.Conv2d(
                mid_chs,                                # 96
                mid_chs,                                # 96
                dw_kernel_size,                         # 5
                stride=stride,                          # 1
                padding=(dw_kernel_size - 1) // 2,      # 2
                groups=mid_chs,                         # 96
                bias=False,
            )
    ...
    def forward(self, x):
        ...
        x = self.ghost1(x)                              # 192 --> 96        特征图尺度不变
        ...
        x = self.ghost2(x)                              # 96 --> 96

        x += self.shortcut(residual)                    # 96 + (192-->96) = 96
        return x
