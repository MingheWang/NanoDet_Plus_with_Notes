...
class Integral(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        shape = x.size()    # shape=(B,N,4*(n+1))
        x = F.softmax(x.reshape(*shape[:-1], 4, self.reg_max + 1), dim=-1)          # shape=(B,N,4,n+1)
        x = F.linear(x, self.project.type_as(x)).reshape(*shape[:-1], 4)            # (B,N,4,n+1)*(n+1,)= (B,N,4) --reshape--> (B,N,4)
        return x
