...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        if self.with_norm:
            ...
            self.norm_name, norm = build_norm_layer(norm_cfg, norm_channels)         # "bn", nn.BatchNorm2d
        ...
        if self.activation:
            self.act = act_layers(self.activation)                                    # nn.LeakyReLU(negative_slope=0.1, inplace=True)
...
class DepthwiseConvModule(nn.Module):
    def __init__(
        ...
        dilation=1,
        bias="auto",
        norm_cfg=dict(type="BN"),       # dict(type="BN")
        activation="ReLU",              # "LeakyReLU"
        inplace=True,
    ...
    def forward(self, x, norm=True):
        for layer_name in self.order:                       # order=("depthwise", "dwnorm", "act", "pointwise", "pwnorm", "act")
            if layer_name != "act":
